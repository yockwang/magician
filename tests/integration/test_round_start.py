...
def test_all_players_hand_stone():
    ...
    assert player_hand_stone.get_all_players_hand_stone(except_input_name) == assigned_stones

    secret_stone = all_stone[:4]
    last_stone = all_stone[4:]

    
    store_warehouse_secret_stone = warehouse_repository(except_input_name)
    ...
    store_warehouse_stone = warehouse_repository(except_input_name)
